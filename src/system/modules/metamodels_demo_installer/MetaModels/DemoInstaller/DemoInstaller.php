...
use MetaModels\DemoInstaller\Tasks\Database\DropTable;
use MetaModels\DemoInstaller\Tasks\Database\Import;
use MetaModels\DemoInstaller\Tasks\Database\TruncateTable;
use MetaModels\DemoInstaller\Tasks\File\InstallFile;
use MetaModels\DemoInstaller\Tasks\File\RemoveFile;
...
class DemoInstaller extends \BackendModule
{
	...
	protected function checkPrerequisities()
	{
		...
		if (is_array($this->config['needed_core']))
		{
			if (!in_array(VERSION, $this->config['needed_core']))
			{
				$arrMissing[] = 'Contao ' . $this->config['needed_core'];
			}
		}
		elseif (VERSION <> $this->config['needed_core'])
		{
			$arrMissing[] = 'Contao ' . $this->config['needed_core'];
		}
	...
	{
		$objTables = $this->Database->executeUncached(sprintf(
		...
		while ($objTables->next())
		{
			...
			{
				$arrTasks[] = new DropTable($this, $objTables->table_name);
			}
			elseif($objTables->TABLE_ROWS>0)
			{
				if ($objTables->table_name != 'tl_session')
				{
					$arrTasks[] = new TruncateTable($this, $objTables->table_name, $objTables->TABLE_ROWS);
				}
			}
		}
	...
	protected function determineRemoveFileTasks($strFolder)
	{
		...
		{
			$arrTasks[] = new RemoveFile($this, $strFile);
		}
	...
	protected function determineSubInstall($folders, $destination)
	{
		...
		foreach ($folders as $dir)
		{
			...
			{
				$arrTasks[] = new InstallFile($this, $file, $destination . substr($file, $len));
			}
		}
	...
	protected function runInstall()
	{
		...
		\Database::getInstance()->prepare("UPDATE tl_session SET pid=? WHERE pid=?")
